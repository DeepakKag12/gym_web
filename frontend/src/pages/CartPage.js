...
export default function CartPage() {
  ...
  if (cart.length === 0) return (
    <div className="min-h-screen bg-[#0a0a0f] pt-16 flex flex-col items-center justify-center gap-5">
      ...
      <p className="text-gray-500 text-sm">Browse our store and add items to cart</p>
            ...
            {cart.map((item, i) => (
              <div key={i} className="glass rounded-2xl p-4 flex items-center gap-4">
                ...
                <div className="flex items-center border border-white/10 rounded-xl overflow-hidden bg-white/5">
                  ...
                  <span className="text-white font-bold px-2 text-sm min-w-[24px] text-center">{item.qty}</span>
                  <button onClick={() => updateQty(item._id, item.flavor, item.weight, item.qty + 1)} className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-colors"><Plus size={13} /></button>
                </div>
                <div className="text-white font-bold text-sm min-w-[60px] text-right">₹{((item.discountPrice || item.price) * item.qty).toFixed(0)}</div>
                <button onClick={() => removeFromCart(item._id, item.flavor, item.weight)} className="text-gray-600 hover:text-red-400 transition-colors ml-1"><Trash2 size={16} /></button>
              </div>
            ))}
          </div>
              ...
              <div className="space-y-2 mb-5">
                {cart.map((item, i) => (
                  <div key={i} className="flex justify-between text-gray-500 text-sm">
                    ...
                    <span className="text-gray-300 font-medium">₹{((item.discountPrice || item.price) * item.qty).toFixed(0)}</span>
                  </div>
                ))}
              </div>
